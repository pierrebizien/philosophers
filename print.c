...
/*   Updated: 2023/04/04 13:35:50 by pbizien          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_is_eating(t_philo *tmp)
{
	...
	gettimeofday(&tv, NULL);
	tmp->last_meal = (tv.tv_usec) + (tv.tv_sec * 1000000);
...
void	ft_print_is_sleeping(t_philo *tmp)
{
	...
	printf("\e[36;49;1m%lld %d is sleeping \e[0m\n", ((tv.tv_usec) + (tv.tv_sec * 1000000) - tmp->data->ts), tmp->num);
	pthread_mutex_unlock(tmp->mutex_print);
	ft_time_sleeping(TIME_TO_SLEEP, (tv.tv_usec) + (tv.tv_sec * 1000000));
